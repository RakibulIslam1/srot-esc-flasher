...
// 4-Way interface modes. The mode is derived from the ESC's device signature at
// DeviceInitFlash (see Check_4Way) exactly the way Betaflight's serial_4way.c
// Connect() does — the host (esc-configurator) trusts this byte to pick which ESC
// family code path to use, so reporting the wrong one breaks everything downstream.
#define imC2      0
#define imSIL_BLB 1
#define imATM_BLB 2
#define imSK      3
...
// (mode constants imC2 / imSIL_BLB / imATM_BLB / imSK / imARM_BLB are defined above)
// PARAM: uint8_t Mode
